...
class Ball {
    ...
    collisionWithWall(wall) {
        ...
        if (xv + ballWidth + (this.velocityX * this.directionX) > wall.x && 
        ...
        if (xv + ballWidth > (wall.x + wall.velocityX)  && 
    ...
    draw(ctx) {
        ...
        ctx.beginPath();
        ctx.ellipse(this.x, this.y, this.radius, this.radius, Math.PI / 4, 0, 2 * Math.PI);
        ctx.fill();
...
class UI {
    ...
    draw(ctx) {
        ...
        if (currentState == Waiting) {
            ...
            ctx.fillText("গেম খেলতে স্পেস চাপুন!", this.width/2, this.height/2);
            ...
            ctx.textAlign = "left";
            ctx.fillText("লাইফঃ ", 20, 30);

            for (let i = 0;i < this.game.life;i++) {
                ctx.fillStyle = 'white';
                ctx.beginPath();
                ctx.ellipse(100 + (35 * i), 23, 7, 7, Math.PI / 4, 0, 2 * Math.PI);
                ctx.fill();
            ...
            ctx.font = "bold 20px Arial";
            ctx.textAlign = "right";
            ctx.fillText("বর্তমান স্কোরঃ " + this.game.totalScore, this.width - 30, 30);
            ...
            if (currentState == Paused) {
                ...
                ctx.textAlign = "center";
                //ctx.filter = "none";
                ctx.fillText("খেলায় ফেরত যেতে স্পেস চাপুন!", this.width/2, this.height/2);
            } else {
                if (this.game.ball.isAttached()) {
                    ctx.fillStyle = 'white';
                    ctx.font = "bold 30px Arial";
                    ctx.textAlign = "center";
                    ctx.fillText("বল ফায়ার করতে ↑ চাপুন!", this.width/2, this.height/2);
                }
            }
            ...
            ctx.font = "bold 40px Arial";
            ctx.textAlign = "center";
            ctx.fillText("আপনি হেরে গেছেন :(", this.width/2, (this.height/2) - 20);

            // Show the total score
            ctx.fillStyle = 'white';
            ctx.font = "20px Arial";
            ctx.textAlign = "center";
            ctx.fillText("আপনার সর্বমোট স্কোরঃ " + this.game.totalScore, this.width/2, (this.height/2) + 20);
        } 
        else if (currentState == GameComplete) {
            ctx.fillStyle = 'white';
            ctx.font = "bold 40px Arial";
            ctx.textAlign = "center";
            ctx.fillText("আপনি জিতে গেছেন!!!!", this.width/2, (this.height/2) - 20);
...
class Game {
    ...
    keyUp(key) {
        if (key == 'Space') {
            ...
            } else if (this.currentState == GameOver || this.currentState == GameComplete) {
                this.newGame();
            }
        } else if (key =='ArrowUp') {
            this.ball.launch();
...
window.addEventListener('load', () => {
    const gameWorldElement = document.getElementById("gameworld");
    const ctx = gameWorldElement.getContext("2d");

    gameWorldElement.width = 1000;
    gameWorldElement.height = 650;

    const game = new Game(gameWorldElement.width, gameWorldElement.height);

    mainLoop = () => {
        ctx.clearRect(0, 0, gameWorldElement.width, gameWorldElement.height);
        game.update();
        game.draw(ctx);
